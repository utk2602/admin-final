...
export default function DomainsPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    setError(null)
    const response = await fetchDomainData(selectedDomain, 2, selectedStatus, lastEvaluatedKey)
    ...
    try {
      ...
      setStudentsData((prevData) =>
        prevData.map((student) =>
          student.email === email ? { ...student, qualification_status1: newStatus } : student
        )
      );
                        ...
                        >
                          ...
                          <td className="p-3">
                            <span className={`px-2 py-1 rounded text-xs ${
                              student.qualification_status1 === 'qualified' ? 'bg-green-900 text-green-300' :
                              student.qualification_status1 === 'pending' ? 'bg-yellow-900 text-yellow-300' :
                              student.qualification_status1 === 'unqualified' ? 'bg-red-900 text-red-300' :
            ...
            {selectedStudent && (
              <div className="flex-1 overflow-hidden flex flex-col">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <div className="bg-gray-700 px-3 py-1 rounded-full flex items-center gap-2">
                            ...
                            <span className={`text-white font-bold ${
                              selectedStudent.qualification_status1 === 'qualified' ? 'text-green-300' :
                              selectedStudent.qualification_status1 === 'pending' ? 'text-yellow-300' :
                              selectedStudent.qualification_status1 === 'unqualified' ? 'text-red-300' :
                              'text-gray-300'
                            }`}>
                              {selectedStudent.qualification_status1 || 'unmarked'}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                  ...
                  {/* Round 2 Responses */}
                  {selectedStudent.round2 && selectedStudent.round2.length > 0 && (
                    <div>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-[#f4b41a]">Round 2 Responses</h3>
                      <div className="flex items-center text-sm bg-gray-800 px-3 py-1 rounded-full">
                        <Info size={14} className="mr-1 text-[#f4b41a]" />
                        <span className="text-gray-300">{selectedStudent.round2.length} Links</span>
                      </div>
                    </div>
